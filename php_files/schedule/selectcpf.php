    ...
    if ($testcpf = mysqli_query($mysqli,"SELECT cpf,contacomigo FROM holder WHERE cpf = '$valor'")){
        if((mysqli_num_rows ($testcpf) < 1 )){
            ...
            if (($cpf == $valor)&&($contacomigo==1)) {
                echo "CPF Correto".'<br/><br/> <button type="submit" value="Agendar" id="btn_schedule" name="btn_schedule" class="btn btn-primary">Agendar</button><br />';
            } elseif ($cpf != $valor){
                echo "CPF Incorreto".'<br/><br/> <button type="submit" value="Agendar" id="btn_schedule" name="btn_schedule"  class="btn btn-danger" disabled>Agendar</button><br />';
