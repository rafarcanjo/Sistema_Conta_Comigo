...
BACKUP VALIDAÇÃO 1
// Email Validation
            if ($result = mysqli_query($mysqli, "SELECT * FROM users WHERE `email` like '$email'")){
                ...
                echo "Falha na consulta do Email"; exit();
            }

            //Password Validation
            if ($result = mysqli_query($mysqli, "SELECT * FROM users WHERE `password` like '$password'")){
                if(mysqli_num_rows ($result) > 0 ){
                ...
                echo "Falha na consulta da Senha"; exit();
            }